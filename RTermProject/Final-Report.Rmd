...
# Adding Shiny App
ui_hyp1 <- fluidPage(
  titlePanel('Input for prediction of likes, Select \'Predict\' tab to view the prediction.'),
  sidebarLayout(
    sidebarPanel(
      numericInput('commentCount_hyp1','Enter number of comments', value = 3, min = 0, max = 999999999999),
      numericInput('views_hyp1','Enter number of views', value = 3, min = 0, max = 999999999999),
...
As observed the high R squared value, explains that the prediction is accurate up to 81.4% of the time. We arrive at this value, because the closer the R squared value is to 1.0 the closer, the closer it is to being perfectly accurate. There are many parameters to assess the model, we've chosen to go ahead with R squared method. The other three summarized variables.
  If you look closely, we've considered the accuracy of the linear model produced when it used with testing dataset, this is because we wanted the model to be unbiased. 
...
ui_hyp2 <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      numericInput('commentCount_hyp2','Enter number of comments', value = 3, min = 0, max = 999999999999),
      numericInput('views_hyp2','Enter number of views', value = 3, min = 0, max = 999999999999),
...
server_hyp2 <- function(input,output,server){
  button_press <- eventReactive(input$predict_hyp2, {
      ...
      paste('For the given number of comment count and views, the number of dislikes predicted is = ', answer)
  })
...
As observed the high R squared value, explains that the prediction is accurate up to 78.8% of the time. We arrive at this value, because the closer the R squared value is to 1.0 the closer, the closer it is to being perfectly accurate. There are many parameters to assess the model, we've chosen to go ahead with R squared method. The other three summarized variables.
  If you look closely, we've considered the accuracy of the linear model produced when it used with testing dataset, this is because we wanted the model to be unbiased.
...
ui_hyp3 <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      numericInput('likes_hyp3','Enter number of comments', value = 3, min = 0, max = 999999999999),
      numericInput('views_hyp3','Enter number of views', value = 3, min = 0, max = 999999999999),
...
As observed the high R squared value, explains that the prediction is accurate up to 70.9% of the time. We arrive at this value, because the closer the R squared value is to 1.0 the closer, the closer it is to being perfectly accurate. There are many parameters to assess the model, we've chosen to go ahead with R squared method. The other three summarized variables.
  If you look closely, we've considered the accuracy of the linear model produced when it used with testing dataset, this is because we wanted the model to be unbiased.
...
## Conclusion:
  ...
  We found this dataset on Kaggle.com, we came up with a novel approach of predicting the number of likes, dislikes and comment count. We learnt that we need to use linear regression depending on the scatter plot we visualize during the exploratory data analysis, density plot we observe, to know the distribution of the response variables. We later introduced correlation heat map, although the idea of correlation coefficient matrix is not a new idea, we found that most of the visualizations on correlation although informative, requires a bit more effort to understand, we simplified it further to make it look more readable to the audience and in an attempt to increase the audience base for the analysis. To make it more interactive, we have used Shiny. This makes it easier for the user to use our model and interact in real time, by providing predictions in real time. Overall, we have made an attempt to make the information in our analysis more reproducible, redistributable and novel.
